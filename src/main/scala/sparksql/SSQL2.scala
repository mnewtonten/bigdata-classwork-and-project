...
object SSQL2 extends App{
  ...
  val data = spark.read.schema(schema).option("header", true).option("delimiter", "\t").csv("/data/BigData/bls/la/la.data.51.Texas")
  ...
  val combinedSHIT = selURate.join(selLForce, Seq("ID","year","period")).sort('value.desc)
