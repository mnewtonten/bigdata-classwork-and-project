...
object SSQL4 extends App{
  ...
  //val data = spark.read.schema(schema).option("header", true).option("delimiter", "\t").csv("/data/BigData/bls/la/la.data.concatenatedStateFiles")
  ...
  val seriesData = spark.read.schema(secondSchema).option("header",true).option("delimiter", "\t").csv("/data/BigData/bls/la/la.series").cache()
  ...
  val groupedSD = seriesData.groupBy('srd_code).count.sort('count.desc).show
